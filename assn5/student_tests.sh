...
@test "Multiple Commands with Redirection: echo and cat" {
    run ./dsh <<EOF
echo "test multi-line" > multi_test.txt
cat multi_test.txt
EOF
    [ "$status" -eq 0 ]
    grep -q "test multi-line" multi_test.txt
}
...
@test "Chained Redirection and Pipes: cat test_output.txt | sort | uniq > final_output.txt" {
    run ./dsh <<EOF
cat test_output.txt | sort | uniq > final_output.txt
EOF
    [ "$status" -eq 0 ]
    [ -f final_output.txt ] # Ensure file was created
}
